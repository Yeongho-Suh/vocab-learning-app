...
5. 다음 갱신 시간 표시
6. **모바일 최적화:**
   - Mobile First 반응형 레이아웃 (CSS media query / Tailwind 반응형 클래스)
   - 모바일 화면 기준 폰트 크기 및 카드 크기 조정
   - 터치 친화적 UI (버튼 크기, 간격)
   - viewport meta 태그 설정 (`width=device-width, initial-scale=1`)
   - 가로/세로 모드 모두 대응
   - 빠른 로딩 (이미지 최소화, 경량 페이지)
...
| **Meaning (Kor)** | Text | 한글 뜻 | `풍부한, 많은` |
| **Meaning (Eng)** | Text | 영어 뜻 | `existing in large quantities` |
| **Example** | Text | 예문 | `The region has abundant natural resources.` |
| ID | - | Notion이 자동 생성 | - |
